...
- `POST /api/compress-image` — `sharp`; re-encodes JPEG/WebP with a quality level (medium/extreme/ultra, ultra also downscales), quantizes PNG via `palette: true`, other formats (gif/bmp/tiff/heic) are normalized to JPEG since they have no useful lossy quality knob
